...
class Input:
    ...
    _event_mouse = {}
    _last_mouse = {}
    _mouse_position = vectors.Vector2(0, 0)

    @classmethod
    def _key_press_event(cls, event):
        """
        ...
        """
        if not cls._event_keys.get(event.keysym_num, False):
            cls._event_keys[event.keysym_num] = True

    @classmethod
    def _key_release_event(cls, event):
        if cls._event_keys.get(event.keysym_num, False):
            cls._event_keys[event.keysym_num] = False

    @classmethod
    def _mouse_event(cls, event):
        cls._mouse_position_event(event)

    @classmethod
    def _mouse_click_event(cls, event):
        """
        ...
        event.widget = The widget in which the event has occurred
        """
        if not cls._event_mouse.get(event.num, False):
            cls._event_mouse[event.num] = True

    @classmethod
    def _mouse_release_event(cls, event):
        if cls._event_mouse.get(event.num, False):
            cls._event_mouse[event.num] = False

    @classmethod
    def _mouse_position_event(cls, event):
        cls._mouse_position.x = event.x
        cls._mouse_position.y = event.y

    @classmethod
    def get_key(cls, key_code):
        return cls._event_keys.get(key_code, False)

    @classmethod
    def get_key_down(cls, key_code):
        return cls.get_key(key_code) and not cls._last_keys.get(key_code, False)

    @classmethod
    def get_key_up(cls, key_code):
        return not cls.get_key(key_code) and cls._last_keys.get(key_code, False)

    @classmethod
    def get_mouse(cls, mouse_button):
        return cls._event_mouse.get(mouse_button, False)

    @classmethod
    def get_mouse_down(cls, mouse_button):
        return cls.get_mouse(mouse_button) and not cls._last_mouse.get(mouse_button, False)

    @classmethod
    def get_mouse_up(cls, mouse_button):
