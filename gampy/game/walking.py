...
class WalkingGame(Game):
    ...
    def init(self):
        ...
        self.add_to_scene(Entity().add_component(MeshRenderer(ground, material2)))
        self.add_to_scene(Entity(rotation=Quaternion(Vector3(1, 0, 0), math.radians(-45)) *
                                          Quaternion(Vector3(0, 0, 1), math.radians(-65))
                                ).add_component(light_components.DirectionalLight(Vector3(1, 0.7, 0), 0.5)))

        camera = Camera(math.radians(70), Window.width / Window.height, 0.1, 100.)
...
class RandomMove(PhysicsObjectComponent):
    ...
    def __del__(self):
        if not RandomMove.is_printed:
            RandomMove.is_printed = True
            print('========Random Move==================================================================',
                  timer,
                  '=====================================================================================', sep='\n')
